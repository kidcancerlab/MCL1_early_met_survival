...
immune_ref <- celldex::ImmGenData() %>%
    ...
    as.Seurat(counts = NULL, data = "logcounts") %>%
    RenameAssays("originalexp" = "RNA") %>%
    process_seurat(run_umap_dims = 1:30)
...
anchor_data <- FindTransferAnchors(object = tumor,
                                   anchor.features = VariableFeatures(tumor),
                                   reference = human_lung_ts,
                                   reference.anchors = 2000,
                                   dims = 1:20)
