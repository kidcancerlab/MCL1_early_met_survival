...
figure_2c <-
  ggplot() +
  geom_bar(data = primary_vs_early,
           aes(x = -1 * order,
               y = NES,
               fill = NES > 0),
           stat = "identity",
  ...
  geom_hline(yintercept = 0,
             color = "black",
             linewidth = 0.5) +
  geom_text(data = primary_vs_early,
            aes(x = -1 * order,
                y = label_y * 4,
                label = pathway),
            size = 3) +                    #Changed 1.5 to 3
  geom_text(data = lab4plot,
            aes(x = x,
                y = y,
                label = label),
            fontface = "bold",
            size = 3) +                        #chaged 2 to 3
  scale_fill_manual(values = c("#377eb8", "#e41a1c")) +
  theme_roberts() +
  theme(strip.background = element_rect(color = "white", fill = "white"),
        strip.text.x = element_text(size = 15, face = "bold"),     #9to10
        axis.text.x = element_text(size = 9),                      #4to6
