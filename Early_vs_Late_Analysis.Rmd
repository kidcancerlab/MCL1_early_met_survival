...
qs::qsave(mouse_lung_ref,
          file = paste0(ref_path, "/GSE151974/mouse_lung_ref.qs"))
...
sobj_list <- parallel::mclapply(seq_len(nrow(sample_list)), function(i) {
    ...
    # Add cell type annotation if mouse
    if (species_pattern == "^mm10_") {
        sobj <-
            annot_cells(sobj)
    }
...
```{r stromal_cell_type_assignment}
...
# Not using this yet, but we might want to use it to fix cell type names
harmonization <-
    c("Alv Mf" =               "Alveolar macrophages",
      "^Art$" =                "Arterial endothelial cells",
      "AT2 1" =                "AT2 cells",
      "AT2 2" =                "AT2 cells",
      "B cell 1" =             "B cells",
      "^Cap$" =                "Capillary endothelial cells",
      "^Cap-a$" =              "Capillary endothelial cells - aerocytes",
...
#     apply(MARGIN = 1, function(x) max(x, na.rm = TRUE))
combined_data$cell_type <-
    str_remove(combined_data$cell_type, " [12]$")

combined_data$cell_type <-
    str_replace_all(combined_data$cell_type,
                harmonization)
...
established_tumor_sobj <- 
    merge(sobj_list[["d28_pos_h"]],
          y = sobj_list[c("d49_pos_h")],
          add.cell.ids = c("Established_metastatic_tumor",
                           "Established_metastatic_tumor"),
          project      =   "established_tumor_sobj")

established_tumor_sobj$group <- "Established_metastatic_tumor"

#make the receiver obj
receiver_sobj <- 
    merge(sobj_list$d14_pos_h,
...
receiver_sobj$cell_type_final <- receiver_sobj$group

receiver_sobj$cell_type_final <- 
    stringr::str_replace_all(receiver_sobj$cell_type_final, "_", " ")
...
Idents(receiver_sobj) <- "cell_type_final"
Idents(sender_receiver_sobj) <- "cell_type_final"
...
ggsave("output/figures/complex_heatmap.pdf",
       width = 12,
       height = 6,
       plot = complex_heatmap)
...
>>>>>>> 499d570c1d7def9aa854ee63f09ca6529dc104e1
