...
qs::qsave(markers_list, file = "output/rdata/markers_list.qs")
qs::qsave(gsea_gene_lists, file = "output/rdata/gsea_gene_lists.qs")
```
...
for (i in seq_len(nrow(category_tibble))) {
    ...
    top5up_down <- list()
    for (comparison in names(gsea_gene_lists)) {
        message(comparison)
        result <-
            fgsea::fgseaMultilevel(kegg_ref,
                                   gsea_gene_lists[[comparison]],
        ...
        write_tsv(as.data.frame(result),
                  file = paste0(markers_list[comparison],
                                ...
                                comparison,
                                ".tsv"))
        kegg_results_list[[comparison]] <- result
    }
