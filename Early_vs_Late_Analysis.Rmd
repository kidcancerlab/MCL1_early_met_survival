...
main_figure_cell_types <-
    c("Dendritic", "Fibroblast", "Alveolar Mac", "Monocyte", "Epithelial")

main_jitter_cell_types <-
    c("Alveolar Mac", "Monocyte", "TAM", "Fibroblast", "Epithelial")
...
gsea_lists <- parallel::mclapply(1:nrow(marker_table), function(i) {
    df <- FindMarkers(human_only,
        ident.1 = marker_table$item1[i],
        ident.2 = marker_table$item2[i],
        min.pct = 0.1,
        logfc.threshold = 0)
    filename1 <- paste0("output/de/",
...
names(gsea_lists) <- paste0(marker_table$item1,
                            "_vs_",
                            marker_table$item2)
...
for (i in seq_len(nrow(category_tibble))) {
    kegg_ref <-
    ...
    kegg_results_list <- parallel::mclapply (names(gsea_lists), function(item) {
        message(item)

        df$gene <- rownames(df)

        gsea_input <-gsea_lists[[item]] %>%
...
names(kegg_results_list) <- names(gsea_gene_lists)

top5up_down <- list()
for(item in names(gsea_gene_lists)) {
    top5up <- kegg_results_list[[item]] %>% slice_head(n = 5)
    top5down <- kegg_results_list[[item]] %>% slice_tail(n = 5)
    temp_top5 <- rbind(top5up, top5down)
    top5up_down[[item]] <- temp_top5 %>%
                             dplyr::select(pathway, NES)
    write_tsv(as.data.frame(result),
              file = paste0(markers_list[comparison],
                            "_",
                            comparison,
                            ".tsv"))
}
