...
## Cell type annotation using Tabula Sapiens dataset
...
cell_assign <-
    SingleR::SingleR(as.SingleCellExperiment(microenv),
...
microenv$cell_type <- cell_assign$labels# %>%
    #str_replace("DC", "Dendritic cells")
microenv$cell_score <- cell_assign$scores %>%
...
DimPlot(microenv,
        ...
        label = TRUE,
        repel = TRUE,
        #cells.highlight = WhichCells(microenv, expression = cell_type == "AT2 1"),
        cols = sample(rainbow(n = length(unique(microenv$cell_type))))) +
    NoLegend()
