...
table(combined_subset$cell_type, combined_subset$group_name) %>%
    as.data.frame() %>%
...
for (factor_name in c("group_name", "sample_description")) {
    ...
    temp_data <-
        ...
        group_by(cell_type, group_name, gene) %>%
        summarize(expression = mean(expression),
                  pct_expr = sum(expression > 0) / n(),
                  .groups = "drop") %>%
        mutate(gene_group = paste(gene, group_name, sep = "\n"))
...
#Dimplot
combined_downsampled_human <- DimPlot(downsample_umap_human_only) +
                            ggtitle(label="Primary and metastatic tumor") +
                            theme_roberts() +
                            theme(plot.title = element_text(hjust = 0.5)) +
                            coord_fixed()
combined_downsampled_human

#split by
downsample_human_umaps <- 
...
downsample_human_umaps$data$group <-
                                factor(x = downsample_human_umaps$data$group,
...
#save the plot
ggsave("output/figures/downsample_human_umaps.pdf",
       width = 6,
       height = 6,
       plot = downsample_human_umaps)
...
#rearrange the panels for umap space
downsample_mouse_umaps$data$group <- 
                                factor(x = downsample_mouse_umaps$data$group,
                                levels = c("Mouse_control_lung",
                                        "Early_mouse_stroma",
                                        "Midpoint_mouse_stroma",
                                        "Late_mouse_stroma"))
...
#save the plot
ggsave("output/figures/downsample_mouse_umaps.pdf",
       width = 6,
       height = 6,
       plot = downsample_mouse_umaps)


#combine plots with patchwork
downsample_combined_plots <- (downsample_human_umaps / downsample_mouse_umaps)
downsample_combined_plots
ggsave("output/figures/downsample_combined_plots.pdf",
       width = 6,
       height = 6,
       plot = downsample_combined_plots)

#all combined
downsample_combined_plots_all <-
                        (combined_downsampled_human + 
                        (downsample_human_umaps / downsample_mouse_umaps)) +
                        plot_layout(widths = c(1, 4))

downsample_combined_plots_all

#save the plot
ggsave("output/figures/downsample_combined_plots_all.pdf",
       width = 6,
       height = 6,
       plot = downsample_combined_plots_all)
...
combined_human_only <- DimPlot(umap_human_only) +
                    ggtitle(label="Primary and metastatic tumor") +
                    theme_roberts() +
                    theme(plot.title = element_text(hjust = 0.5)) +
                    coord_fixed()
...
# perform GSEA
kegg_primary_early <- fgsea::fgseaMultilevel(kegg_ref,
                        gsea_gene_lists$Primary_tumor_vs_Early_metastatic_tumor,
...
#I am here trying to figure out how to make it work in loop
...
Find_markers <- function(x) {
    x <- for (i in 1:nrow(marker_table)) {
        y <- FindMarkers(human_only,
                            ident.1 = marker_table$item1[i],
                            ident.2 = marker_table$item2[i],
                            min.pct = 0.1,
                            logfc.threshold = 0)
