...
qs::qsave(combined_data, file = "output/rdata/combined_data.qs")
```
...
```{r}
#load the sobj
umap_human_only <- qs::qread("output/rdata/human_tumor.qs")
...
#Dimplot
human_umaps <-
    DimPlot(umap_human_only, split.by = "group") +
            theme_robert() +
            theme(plot.title = element_text(hjust = 0.5)) +
            NoLegend() + coord_fixed()

#for mouse
umap_mouse_only <- qs::qread("output/rdata/combined_data.qs")
Idents(umap_mouse_only) <- "cell_type_final"
...
combined_plots_all <-
    (combined_human_only + human_umaps + plot_layout(widths = c(1, 4))) /
    (mouse_umaps_combined + mouse_umaps + plot_layout(widths = c(1, 4)))

#save the plot
ggsave("output/figures/combined_human_only.pdf",
       width = 6,
       height = 6,
       plot = combined_human_only)
